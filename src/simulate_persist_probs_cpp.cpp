...
//   2) simulate_persist_quantiles_cpp()
//
// -----------------------------------------------------------------------------
...
// [[Rcpp::plugins(cpp11)]]
// [[Rcpp::plugins(openmp)]]
// Stage 2 requires an OpenMP-capable compiler and validates the compiled
// runtime before any persistence simulation begins.
...
//
// Important: chunk_size trades memory use against loop/RNG overhead and is part
// of the CRN experiment definition. Changing chunk_size changes which draws are
// assigned to deterministic substreams, so do not change it casually when paired
// comparisons need exact common-random-number alignment.
//
struct CRNContext {
...
// [[Rcpp::export]]
Rcpp::String persist_cpp_contract() {
  return "initial-threshold-v4-validated-quantiles";
}

// [[Rcpp::export]]
Rcpp::List persist_openmp_info() {
#ifdef _OPENMP
  return Rcpp::List::create(
    Rcpp::Named("enabled") = true,
    Rcpp::Named("version") = static_cast<int>(_OPENMP),
    Rcpp::Named("max_threads") = omp_get_max_threads(),
    Rcpp::Named("num_procs") = omp_get_num_procs(),
    Rcpp::Named("dynamic") = static_cast<bool>(omp_get_dynamic())
  );
#else
  return Rcpp::List::create(
    Rcpp::Named("enabled") = false,
    Rcpp::Named("version") = NA_INTEGER,
    Rcpp::Named("max_threads") = 1,
    Rcpp::Named("num_procs") = 1,
    Rcpp::Named("dynamic") = false
  );
#endif
}
...
static inline void simulate_core_crn(
  ...
  // Trajectories start at N0 = K. Persistence means remaining above the
  // quasi-extinction threshold for the full horizon, so K <= ext_thr is
  // structurally non-persistent before any annual transition is simulated.
  if (K <= static_cast<double>(ext_thr)) {
    std::fill(out, out + ctx->n_draws, 0.0);
    return;
  }
...
//==============================================================================
// 4) Export: requested quantiles (CRN default)
//==============================================================================

// [[Rcpp::export]]
Rcpp::NumericVector simulate_persist_quantiles_cpp(
    ...
    const double cap_factor,
    SEXP crn_ctx,
    const Rcpp::NumericVector& quantile_probs) {
  
  const int n_draws = r.size();
  if (sigma.size() != n_draws) Rcpp::stop("r and sigma length mismatch");
  if (!std::isfinite(K) || !(K > 0.0)) Rcpp::stop("K must be finite and > 0");
  if (ext_thr <= 0)            Rcpp::stop("ext_thr must be > 0");
  if (!std::isfinite(cap_factor) || cap_factor < 1.0) Rcpp::stop("cap_factor must be finite and >= 1");
  if (quantile_probs.size() == 0) Rcpp::stop("quantile_probs must not be empty");
  for (R_xlen_t i = 0; i < quantile_probs.size(); ++i) {
    const double p = quantile_probs[i];
    if (!std::isfinite(p) || p < 0.0 || p > 1.0) {
      Rcpp::stop("quantile_probs must contain finite values in [0, 1]");
    }
  }
  for (int i = 0; i < n_draws; ++i) {
    if (!std::isfinite(r[i]) || r[i] <= 0.0) Rcpp::stop("r must contain positive finite values");
    if (!std::isfinite(sigma[i]) || sigma[i] < 0.0) Rcpp::stop("sigma must contain non-negative finite values");
  }
  ...
  Rcpp::NumericVector out(quantile_probs.size());
  for (R_xlen_t i = 0; i < quantile_probs.size(); ++i) {
    out[i] = quantile_type7_inplace(xs, quantile_probs[i]);
  }
  return out;
}
